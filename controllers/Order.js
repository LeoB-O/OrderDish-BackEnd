    ...
    try {
        ...
        if (checkexists) {
            ...
            await order.create({
                id: checkexists["id"],
                price: sum,
                addressid: defaultadid ? defaultadid["dataValues"]["id"] : null,
                state: 0,
                Uid: token
            })
            for (let c of content) {
            ...
            num++;
            let defaultadid = await userad.findOne({where: {Uid: token}, attributes: ["id"]});
            await order.create({id: num, state: 0, price: sum, addressid: defaultadid?defaultadid["dataValues"]["id"]:null, Uid: token});
    ...
    try {
        ...
        if (!oD) {
            ...
            for (let temp of oDF) {
                let temp_c = {};
                temp_c["id"] = temp["fid"];
                let finfo = await  food.findOne({
                    where: {id: temp_c["id"]},
                    attributes: ["Fname", "price", "trueprice"]
                });
